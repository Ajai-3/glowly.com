...
export const editProduct = async (req, res) => {
    try {
        const { productName, brand, description, category, subCategory, variantColor, variantShade, variantRegularPrice, variantSalePrice, variantStockQuantity, removedImages } = req.body;
        const { productId, variantId } = req.params;
        ...
        if (
            ...
            !variantColor || !variantShade || !variantRegularPrice || !variantSalePrice || !variantStockQuantity
        ) {
            return res.status(400).json({ success: false, message: 'Missing required fields' });
        }
        ...
        if (removedImages && removedImages.length > 0 || (variantImages && variantImages.length > 0)) {
            console.log("Updating images...");
            variant.images = updateVariantImages(variant.images, removedImages, variantImages);
...
function updateVariantImages(existingImages, removedImages, newImages) {
    ...
    if (removedImages && removedImages.length > 0 && newImages && newImages.length > 0) {
...
export const addNewVariants = async (req, res) => {
    try {
        ...
        parsedVariants.forEach((variant, index) => {
            if (!variant.color || !variant.shade || !variant.stockQuantity || !variant.regularPrice || !variant.salePrice) {
                throw new Error(`Missing required field in variant ${index + 1}`);
            }

            if (isNaN(variant.stockQuantity) || parseInt(variant.stockQuantity) <= 0) {
                throw new Error(`Invalid stock quantity for variant ${index + 1}`);
            }

            if (isNaN(variant.regularPrice) || parseFloat(variant.regularPrice) <= 0) {
